...
def _manifest(*, hardware: str = "r7a.4xlarge") -> dict[str, object]:
    return {
        ...
        "workloads": {"corpus": "prague-compute-v1"},
        "boundary": "NewL1EvmBlockExecution::execute",
        "gas_schedule": "prague",
    }
...
def test_campaign_comparison_rejects_unavailable_schedule_identity(
    ...
    for directory, revision in ((baseline, "before"), (candidate, "after")):
        ...
        manifest["comparison_factors"] = {
            ...
            "workload": "prague-compute-v1",
            "execution_boundary": "NewL1EvmBlockExecution::execute",
            "gas_schedule": {
                "active_schedule_identity": {
                    "status": "unavailable",
                    "reason": "native worker protocol does not expose a schedule",
                }
            },
        }
        _write_comparison_input(
